...
extern gboolean gapc_cb_refresh_timer (gpointer gp);
extern gboolean gapc_cb_timer_control (gpointer gp);
extern gint gapc_monitor_update (PGAPC_CONFIG pcfg);
extern gint gapc_text_view_clear_buffer (GtkWidget * view);
extern gpointer *gapc_network_thread (PGAPC_CONFIG pcfg);
extern gboolean gapc_cb_h_bar_chart_exposed (GtkWidget * widget, GdkEventExpose * event, gpointer data);
extern GtkWidget *gapc_create_h_barchart (PGAPC_CONFIG pcfg, GtkWidget * vbox,  gchar * pch_hbar_name, gdouble d_percent, gchar * pch_text);
extern gint gapc_net_transaction_service (PGAPC_CONFIG pcfg, gchar * cp_cmd, gchar ** pch);
extern void gapc_cb_config_save (GtkButton * button, gpointer gp);
...
/*  
  ...
  gtk_tooltips_set_tip (pcfg->tooltips, GTK_WIDGET (pcfg->applet), "UPS Initializing...", NULL); 
*/  
