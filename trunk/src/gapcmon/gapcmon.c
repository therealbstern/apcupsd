...
static void     cb_main_interface_button_quit (GtkWidget * button,
                                               PGAPC_CONFIG pcfg);
...
static gboolean cb_util_line_chart_refresh_control (PGAPC_MONITOR pm)
{
  ...
  if ((pm->tid_graph_refresh != 0) && (w != NULL))
  {
      ...
      pch1 = g_strdup_printf
              ("Graphing refresh cycle changed for host %s completed!...",
               pm->pch_host);
...
static gboolean cb_monitor_automatic_refresh (PGAPC_MONITOR pm)
{
  ...
  if (gapc_monitor_update (pm))
  {
    if (w != NULL)
    {
      pch1 = g_strdup_printf ("Automatic refresh for %s complete...", pm->pch_host);
      gtk_statusbar_push (GTK_STATUSBAR (w), pm->i_info_context, pch1);
      g_free (pch1);
    }
  }
                                                      ...
                                                      gint i_monitor)
{
  ...
  g_snprintf (pk.k_use_systray, GAPC_MAX_TEXT, "%s/%d/%s", pkey, i_monitor, "use_systray");
  g_snprintf (pk.k_port_number, GAPC_MAX_TEXT, "%s/%d/%s", pkey, i_monitor, "port_number");
  g_snprintf (pk.k_network_interval, GAPC_MAX_TEXT, "%s/%d/%s", pkey, i_monitor, "network_interval");
  g_snprintf (pk.k_graph_interval, GAPC_MAX_TEXT, "%s/%d/%s", pkey, i_monitor, "graph_interval");
  g_snprintf (pk.k_host_name, GAPC_MAX_TEXT, "%s/%d/%s", pkey, i_monitor, "host_name");
...
static GtkWidget *gapc_util_barchart_create (PGAPC_MONITOR pm, GtkWidget * vbox,
                                             gchar * pch_hbar_name,
                                             gdouble d_percent, gchar * pch_text)
{
  PGAPC_BAR_H     pbar = NULL;
...
static void cb_panel_prefs_handle_cell_edited (GtkCellRendererText * cell,
                                               gchar * path_string, gchar * pch_new,
                                               PGAPC_PREFS_COLUMN pcolumn)
{
  GtkTreeModel   *model;
  GtkTreeIter     iter;
...
static gboolean gapc_panel_preferences_data_model_load (PGAPC_CONFIG pcfg)
{
  ...
  {                             /* should be the regular text key */
    ...
    g_snprintf (k_enabled, GAPC_MAX_TEXT, "%s/%s", (gchar *)monitors->data, "enabled");
    g_snprintf (k_use_systray, GAPC_MAX_TEXT, "%s/%s", (gchar *)monitors->data, "use_systray");
    g_snprintf (k_port_number, GAPC_MAX_TEXT, "%s/%s", (gchar *)monitors->data, "port_number");
    g_snprintf (k_network_interval, GAPC_MAX_TEXT, "%s/%s", (gchar *)monitors->data, "network_interval");
    g_snprintf (k_graph_interval, GAPC_MAX_TEXT, "%s/%s", (gchar *)monitors->data, "graph_interval");
    g_snprintf (k_host_name, GAPC_MAX_TEXT, "%s/%s", (gchar *)monitors->data, "host_name");
    ...
    v_graph_interval = gconf_client_get_float (pcfg->client, k_graph_interval, NULL);
    ...
    gtk_list_store_set (GTK_LIST_STORE (pcfg->prefs_model), &iter,
                        ...
                        GAPC_PREFS_GRAPH, v_graph_interval, GAPC_PREFS_HOST,
                        v_host_name, -1);
/*    g_free (v_host_name); */
...
static GtkWidget *gapc_panel_prefrences_model_init (PGAPC_CONFIG pcfg)
{
  ...
  gtk_tree_view_column_set_cell_data_func (column, renderer_refresh,
                                           ...
                                           GUINT_TO_POINTER (GAPC_PREFS_REFRESH),
                                           NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (treeview), column);
...
static gint gapc_panel_monitor_list_page (PGAPC_CONFIG pcfg, GtkNotebook * notebook)
{
  ...
  label = gtk_label_new ("<span foreground=\"blue\">"
                         "<i>double-click a row to popup information window.</i>"
                         "</span>");
...
static gint gapc_panel_preferences_page (PGAPC_CONFIG pcfg, GtkNotebook * notebook)
{
  ...
  label = gtk_label_new ("<span foreground=\"blue\">"
                         "<i>double-click a columns value to change it.</i>"
                         "</span>");
  gtk_label_set_use_markup (GTK_LABEL (label), TRUE);
  gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_CENTER);
...
*/
static void cb_monitor_interface_button_refresh (GtkWidget * button,
                                                 PGAPC_MONITOR pm)
{
  g_return_if_fail (pm != NULL);
                                                ...
                                                PGAPC_CONFIG pcfg)
{
  ...
  b_ls_valid =
          gapc_util_treeview_get_iter_from_monitor (pcfg->prefs_model, &iter,
                                                    i_monitor);
  if (b_ls_valid)
  {
  ...
  if (b_ls_valid && !b_k_valid && !b_v_valid)
  ...
  if (i_monitor == pcfg->cb_last_monitor_deleted)
  ...
  {    /* add new rec if keys valid -nfound */
    ...
    gtk_list_store_set (GTK_LIST_STORE (pcfg->prefs_model), &iter,
                        ...
                        g_strdup (GAPC_HOST_DEFAULT), GAPC_PREFS_REFRESH,
                        GAPC_REFRESH_DEFAULT, -1);
  ...
  if (b_ls_valid && b_v_valid && b_k_valid)
  {
    ...
    if (g_str_equal (pkey, "enabled"))
    {
      ...
      {
        gtk_widget_show_all (gapc_monitor_interface_create (pcfg, i_monitor, &iter));
      }
      if (!b_value && b_m_enabled && b_active_valid)
      {
        gapc_monitor_interface_destroy (pcfg, i_monitor);
    ...
    if (g_str_equal (pkey, "port_number"))
    {
      ...
      if (i_value != ov_i_port) {
          gtk_list_store_set (GTK_LIST_STORE (pcfg->prefs_model), &iter, GAPC_PREFS_PORT,
                          i_value, -1);
      }
      if ((b_m_enabled) && (b_active_valid))
      {
    ...
    if (g_str_equal (pkey, "host_name"))
    {
      ...
      if ( !g_str_equal ( s_value, ov_s_host) ) {
          gtk_list_store_set (GTK_LIST_STORE (pcfg->prefs_model), &iter, GAPC_PREFS_HOST,
                          g_strdup (s_value), -1);
      }
      if ((b_m_enabled) && (b_active_valid))
      {
...
static gboolean gapc_panel_gconf_watch (PGAPC_CONFIG pcfg)
{
  ...
  pcfg->i_group_id =
          gconf_client_notify_add (pcfg->client, GAPC_CP_GROUP_KEY,
                                   ...
                                   cb_panel_controller_gconf_changed, pcfg, NULL,
                                   &gerror);
  if (gerror != NULL)
  {
    gapc_util_log_app_msg ("gapc_panel_gconf_watch",
                           "gconf_client_notify_add(controller.group) Failed",
                           gerror->message);
    g_error_free (gerror);
    pcfg->i_group_id = 0;
    gerror = NULL;
  ...
  pcfg->i_prefs_id =
          gconf_client_notify_add (pcfg->client, GAPC_MID_GROUP_KEY,
                                   (GConfClientNotifyFunc)
                                   cb_panel_preferences_gconf_changed, pcfg, NULL,
                                   &gerror);
  if (gerror != NULL)
  {
    gapc_util_log_app_msg ("gapc_panel_gconf_watch",
                           "gconf_client_notify_add(prefs.group) Failed",
                           gerror->message);
    g_error_free (gerror);
    pcfg->i_group_id = 0;
    gerror = NULL;
...
static gboolean gapc_panel_gconf_init (PGAPC_CONFIG pcfg)
{
  ...
  /* Defaults are FALSE */
  pcfg->b_use_systray =
          gconf_client_get_bool (pcfg->client, pcfg->pch_gkeys[GAPC_PREFS_SYSTRAY],
                                 NULL);
...
static GtkWidget *gapc_main_interface_create (PGAPC_CONFIG pcfg)
{
  ...
  window = g_object_new (GTK_TYPE_WINDOW, "border-width", 0, "destroy-with-parent",
                         ...
                         GDK_WINDOW_TYPE_HINT_NORMAL, "window-position",
                         GTK_WIN_POS_NONE, NULL);
                                                 ...
                                                 GtkTreeIter * iter)
{
  ...
  g_snprintf (pm->ch_title_info, GAPC_MAX_TEXT, "%s {%s}", GAPC_WINDOW_TITLE, pm->pch_host);
  ...
  g_signal_connect (window, "destroy", G_CALLBACK (cb_monitor_interface_destroy),
                    pm);
  g_signal_connect (window, "delete-event",
...
static void gapc_monitor_interface_destroy (PGAPC_CONFIG pcfg, gint i_monitor)
{
  ...
  if (gapc_util_treeview_get_iter_from_monitor(pcfg->monitor_model, &iter, i_monitor))
  {
    gtk_tree_model_get (pcfg->monitor_model, &iter, GAPC_MON_POINTER, &pm, -1);
  }
