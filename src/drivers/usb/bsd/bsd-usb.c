/*
 *
 *  Interface for apcupsd to *BSD ugen USB driver.
 *
 *   Adam Kropelin, September 2004 
 *
...
static int init_device(UPSINFO *ups, const char* devname)
{
    ...
    if (!hidu_locate_item(my_data->rdesc, UPS_USAGE, -1, -1, -1, HID_KIND_COLLECTION, NULL))
    {
...
int pusb_ups_get_capabilities(UPSINFO *ups, const struct s_known_info* known_info)
{
    ...
    for (i=0; known_info[i].usage_code; i++)
    {
        ...
        if (ci != CI_NONE && !ups->UPS_Cap[ci])
        {
            ...
            rc = hidu_locate_item(my_data->rdesc,
...
int pusb_ups_check_state(UPSINFO *ups)
{
    ...
    for ( ;; ) {
        ...
        if (now.tv_sec > exit.tv_sec ||
            (now.tv_sec == exit.tv_sec && now.tv_nsec/1000 >= exit.tv_nsec/1000))
        {
