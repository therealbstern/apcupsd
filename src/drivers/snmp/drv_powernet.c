...
int powernet_snmp_ups_read_static_data(UPSINFO *ups) {
    ...
    if (data->upsBasicIdent) {
        astrncpy(ups->upsmodel, (const char *)data->upsBasicIdent->upsBasicIdentModel,
                sizeof(ups->upsmodel));
        astrncpy(ups->upsname, (const char *)data->upsBasicIdent->upsBasicIdentName,
                sizeof(ups->upsname));
    ...
    if (data->upsAdvIdent) {
        astrncpy(ups->firmrev, (const char *)data->upsAdvIdent->upsAdvIdentFirmwareRevision,
                sizeof(ups->firmrev));
        astrncpy(ups->birth, (const char *)data->upsAdvIdent->upsAdvIdentDateOfManufacture,
                sizeof(ups->birth));
        astrncpy(ups->serial, (const char *)data->upsAdvIdent->upsAdvIdentSerialNumber,
                sizeof(ups->serial));
