...
// Not defined in Windows headers
...
// Perform a control transfer, libusb style
static int usb_control_msg(WINUSB_INTERFACE_HANDLE fd, int requesttype, 
                           ...
                           unsigned char *bytes, int size, int timeout)
{
   ...
   ULONG actlen = 0;
   if (!WinUsb_ControlTransfer(fd, sp, bytes, size, &actlen, NULL))
      return -1;

   return actlen;
}

// Fetch a string descriptor, libusb style
static int usb_get_string_simple(WINUSB_INTERFACE_HANDLE fd, int index, 
                                 char *dest, int len)
{
   unsigned char temp[MAXIMUM_USB_STRING_LENGTH];

   ULONG actlen = 0;
   if (!WinUsb_GetDescriptor(fd, USB_STRING_DESCRIPTOR_TYPE, index, 0x0409, 
   ...
   unsigned int i, j;
   for (i = 2, j = 0; i < actlen && j < (len-1); i+=2, ++j)
      dest[j] = temp[i];
   dest[j] = '\0';
...
int usb_get_intf_descriptor(WINUSB_INTERFACE_HANDLE fd, unsigned char type,
                            unsigned char index, void *buf, int size)
{
   memset(buf, 0, size);

   return usb_control_msg(fd, USB_ENDPOINT_IN | USB_RECIP_INTERFACE,
                         USB_REQ_GET_DESCRIPTOR,
                         (type << 8) + index, 0, (unsigned char *)buf, size, 1000);
}
 ...
 */
unsigned char *hidu_fetch_report_descriptor(WINUSB_INTERFACE_HANDLE fd, int *rlen)
{
   unsigned char *ptr;
   int rdesclen;
 ...
 */
int hidu_get_report(WINUSB_INTERFACE_HANDLE fd, hid_item_t *item, unsigned char *data, int len)
{
   int actlen;
 ...
 */
int hidu_set_report(WINUSB_INTERFACE_HANDLE fd, hid_item_t *item, unsigned char *data, int len)
{
   int actlen;
 ...
 * on failure.
 */
const char *hidu_get_string(WINUSB_INTERFACE_HANDLE fd, int index)
{
   int rc;
