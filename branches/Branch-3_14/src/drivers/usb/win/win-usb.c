 ...
 * win-usb.c
 *
 * Windows USB driver utilizing the WinUSB subsystem
 */
...
DLL_DECLARE(WinUsb_Initialize);
DLL_DECLARE(WinUsb_Free);
DLL_DECLARE(WinUsb_GetAssociatedInterface);
DLL_DECLARE(WinUsb_GetDescriptor);
DLL_DECLARE(WinUsb_QueryInterfaceSettings);
DLL_DECLARE(WinUsb_QueryDeviceInformation);
DLL_DECLARE(WinUsb_SetCurrentAlternateSetting);
DLL_DECLARE(WinUsb_GetCurrentAlternateSetting);
DLL_DECLARE(WinUsb_QueryPipe);
DLL_DECLARE(WinUsb_SetPipePolicy);
DLL_DECLARE(WinUsb_GetPipePolicy);
DLL_DECLARE(WinUsb_ReadPipe);
DLL_DECLARE(WinUsb_WritePipe);
DLL_DECLARE(WinUsb_ControlTransfer);
DLL_DECLARE(WinUsb_ResetPipe);
DLL_DECLARE(WinUsb_AbortPipe);
DLL_DECLARE(WinUsb_FlushPipe);
...
typedef struct s_usb_data {
   HANDLE hnd;                     /* Generic CreateFile handle for device */
   WINUSB_INTERFACE_HANDLE fd;     /* WinUSB handle for device */
   ...
   SP_DEVICE_INTERFACE_DATA *interfaceData)
{
   ...
   {
      free(detailData);
      Dmsg1(100, "SetupDiGetDeviceInterfaceDetail fails: %d\n", GetLastError());
      return 0;
   }

   Dmsg1(100, "Considering Device: \"%s\"\n", detailData->DevicePath);

   // Open generic handle to device
   HANDLE hnd = CreateFile(detailData->DevicePath,
   ...
   if (!WinUsb_Initialize(hnd, &fd))
   {
      CloseHandle(hnd);
      free(detailData);
      Dmsg1(100, "WinUsb_Initialize fails: %d\n", GetLastError());
      return 0;
   }

   // Fetch the report descriptor
   int rdesclen;
   unsigned char *rdesc = hidu_fetch_report_descriptor(fd, &rdesclen);
   if (!rdesc)
   {
      WinUsb_Free(fd);
      CloseHandle(hnd);
   ...
   // Found a device and initialized from it; we're good to go
   Dmsg0(100, "Found compatible APC UPS device\n");
   my_data->fd = fd;
   my_data->hnd = hnd;
   astrncpy(ups->device, detailData->DevicePath, sizeof(ups->device));
   free(detailData);
   return 1;
}
...
int open_usb_device(UPSINFO *ups)
{
   ...
   // Initialize DLL functions on the first call
   ...
   if (!deviceInfo)
   {
      Dmsg0(100, "No matching device interfaces found\n");
      return 0;
   }
   ...
   while(1)
   {
      ...
      if (!SetupDiEnumDeviceInterfaces(
            deviceInfo, NULL, &APCUPSD_DEVICE_GUID, 0, &interfaceData) ||
          init_device(ups, deviceInfo, &interfaceData))
      {
         break;
      }
   }
...
static int usb_link_check(UPSINFO *ups)
{
   ...
   for (tlog = LINK_RETRY_INTERVAL * 2; comm_err; tlog -= (LINK_RETRY_INTERVAL)) {
      ...
      if (my_data->fd)
      {
         WinUsb_Free(my_data->fd);
         CloseHandle(my_data->hnd);
         my_data->fd = my_data->hnd = NULL;
         hid_dispose_report_desc(my_data->rdesc);
...
static int usb_interrupt_read(WINUSB_INTERFACE_HANDLE fd, int ep, unsigned char *bytes, int size, int timeout)
{
   // Set timeout on endpoint
   // It would be more efficient to use async i/o here...
   ULONG tmp = timeout;
   if (!WinUsb_SetPipePolicy(fd, ep, PIPE_TRANSFER_TIMEOUT, sizeof(tmp), &tmp))
   {
      Dmsg1(200, "WinUsb_SetPipePolicy fails: %lu\n", GetLastError());
      return -EINVAL;
   }

   // Perform transfer
   tmp = 0;
   if (!WinUsb_ReadPipe(fd, ep, bytes, size, &tmp, NULL))
   {
      tmp = GetLastError();
      Dmsg1(200, "WinUsb_ReadPipe fails: %lu\n", tmp);
...
int pusb_ups_check_state(UPSINFO *ups)
{
   ...
   while (!done) {
      ...
      Dmsg1(200, "Timeout=%d\n", timeout);
      retval = usb_interrupt_read(my_data->fd, USB_ENDPOINT_IN|1, buf, sizeof(buf), timeout);
